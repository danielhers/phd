...
\only<2>{
\vspace{2cm}
The task will involve UCCA parsing in English, German and French
in different domains (\textit{Wikipedia, 20K Leagues Under the Sea}).
\vfill
\begin{center}
...
\only<2>{
\vspace{2cm}
Baseline for the task: TUPA \cite{hershcovich2017a}, a freely available transition-based UCCA parser.
\hfill\url{bit.ly/tupademo}}
\end{frame}
...
Already developed a parser for AMR/SDP/UD?
We have protocols to convert them to UCCA \cite{hershcovich2018multitask},
so please consider making a submission!
