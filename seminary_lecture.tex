...
\author[Daniel Hershcovich]{Daniel Hershcovich \\ Advisors: Ari Rappoport and Omri Abend}
...
\begin{tikzpicture}[level distance=15mm, sibling distance=28mm, ->, draw=Indigo, thick,
  ...
  \tikzstyle{word} = [color=black]
    ...
    \node (ROOT) [fill=Indigo, circle] {}
      ...
      } edge from parent [alt=<4>{red}{Indigo}] node[above] {A} }
      ;
  ...
  \tikzstyle{word} = [color=black]
    \node[anchor=west,font=\sffamily\LARGE,Indigo] at (-6,0) {UCCA};
      \node (ROOT) [circle] {}
        child {node [circle] {}
        {
          child {node [word] {From} edge from parent node[left] {R}}
          child {node [word] {the} edge from parent node[left] {E}}
          child {node [word] {{\color{white}F}moment} edge from parent node[right] {C}}
...
\begin{itemize}
 ...
 \item Multitask learning improves UCCA parsing by shared generalizations.
 \item Divergences between UCCA and UD impact the utility of syntax for semantic parsing
       and the value of each for NLP applications.
\end{itemize}
\end{frame}
