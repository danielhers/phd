...
\vfill
    ...
    \begin{dependency}[line width=1.5pt]
    	...
    	They \^ need \^ to \^ take \^ a \^ short \^ break \\
        \end{deptext}
...
Rapid and intuitive annotation of linguistic semantic phenomena. \\
\onslide<2->{
...
\begin{adjustbox}{center}
            \begin{tikzpicture}[level distance=7mm, sibling distance=6mm,
                ...
                \draw (graduation John u) to node[auto] {\scriptsize A} (John u);
                \end{scope}
                \onslide<2->{
...
\hspace*{1cm}
  \begin{tikzpicture}[level distance=16mm, sibling distance=28mm, ->,
  level 2/.style={level distance=9mm, sibling distance=8mm},
  level 4/.style={level distance=14mm, sibling distance=18mm},
  edge from parent/.append style={nodes={font=\scriptsize}}]
  \tikzstyle{word} = [font=\rmfamily,color=black]
    \node (ROOT) [fill=blue, circle] {}
      child {node (They) [word] {They} edge from parent node[left] {A}}
      child {node [word] {need} edge from parent node[left] {P}}
      child {node (totakeashortbreak) [fill=blue, circle] {}
      {
        child {node [word] {to} edge from parent node[left] {F}}
        child {node (takeashortbreak) [fill=blue, circle] {}
        {
          child {node (takeabreak) [fill=blue, circle] {}
          {
            child {node [word] {take} edge from parent node[above] {C}}
      ...
      ;
    \draw[bend left,dashed,->,orange,very thick] (totakeashortbreak) to node [auto] {\scriptsize A} (They);
    \draw[bend left,->,red,very thick] (takeashortbreak) to node [auto] {\scriptsize D} (short);
    \node[visible on=<2->] at (6,-0.4) {\Large ----- primary edge};
    \node[visible on=<2->] at (6,-1.4) {\Large - - - remote edge};
\end{tikzpicture}
...
\scalebox{.8}{
  \begin{tikzpicture}[level distance=12mm, sibling distance=2cm, ->,
    ...
    \node (ROOT) [fill=black, circle] {}
      child {node (John) {John} edge from parent node[left] {A}}
      child {node {decided} edge from parent node[left] {P}}
      child {node (totakeaquickshower) [fill=black, circle] {}
      {
        child {node {to} edge from parent node[left] {F}}
        child {node [fill=black, circle] {}
        {
          child {node {take} edge from parent node[left] {C}}
          child {node {a} edge from parent node[right] {F}}
          child {node (quick) {quick} edge from parent[white]}
          child {node {shower} edge from parent node[right] {C}}
        } edge from parent node[right] {P} }
      } edge from parent node[left] {A} }
      ;
...
\begin{tikzpicture}[every node/.append style={font=\rmfamily}, circle]
    ...
    \node[anchor=west] at (3,0.25) {\small They};
    \node[anchor=west] at (4,0.25) {\small need};
    \node[anchor=west] at (5,0.25) {\small to};
    \node[anchor=west] at (6,0.25) {\small take};
    \node[anchor=west] at (7,0.25) {\small a};
    \node[anchor=west] at (8,0.25) {\small short};
    \node[anchor=west] at (9,0.25) {\small break};
\end{tikzpicture}
...
\begin{tikzpicture}[every node/.append style={font=\rmfamily}]
    ...
    \only<29> \node[anchor=west] at (0,0.25) {\small They};
    \only<1-3,7-24> \node[anchor=west] at (1,0.25) {\small They};
    \only<4-5> \node[anchor=west] at (1,0.25) {\small need};
    \only<3>   \node[anchor=west] at (2,0.25) {\small need};
    \only<8-9>  \node[anchor=west] at (2,0.25) {\small to};
    \only<13-14> \node[anchor=west] at (3,0.25) {\small take};
    \only<18-19> \node[anchor=west] at (4,0.25) {\small a};
    \only<21> \node[anchor=west] at (4,0.25) {\small short};
    \only<22-23> \node[anchor=west] at (3,0.25) {\small short};
    \only<32-> \node[anchor=west] at (1,0.25) {\small break};
    ...
    \only<4-6> \node[anchor=west] at (4,0.25) {\small They};
    \only<25-28> \node[anchor=west] at (7,0.25) {\small They};
    \only<-2>  \node[anchor=west] at (4,0.25) {\small need};
    \only<-7> \node[anchor=west] at (5,0.25) {\small to};
    \only<-12> \node[anchor=west] at (6,0.25) {\small take};
    \only<-17> \node[anchor=west] at (7,0.25) {\small a};
...
\begin{tikzpicture}[level distance=15mm, sibling distance=2cm, ->,
    ...
    \node(ROOT)[fill=black, circle, visible on=<1->] at (3,0) {}
      ...
      child [visible on=<5->,alt=<5>{draw=red}{}] {node (need) {need} edge from parent node [left] {P}}
      child [visible on=<9->] {node (totakeashortbreak) [fill=blue, circle] {}
      {
        child [visible on=<9->] {node (to) {to} edge from parent node [left] {F}}
        child [visible on=<14->] {node (takeabreak) [fill=red, circle] {}
        {
          child [visible on=<14->] {node (take) {take} edge from parent node [right] {C}}
          ...
          child [visible on=<32->,alt=<32>{draw=red}{}] {node (break) {break} edge from parent node [right] {C}}
        } edge from parent [draw=none]}
      } edge from parent [draw=none]}
      ;
    \draw[visible on=<17->,alt=<17>{draw=red}{}] (totakeashortbreak) to node [left] {\scriptsize P} (takeabreak);
    \draw[visible on=<26->,alt=<26>{draw=red}{}] (ROOT) to node [left] {\scriptsize A} (totakeashortbreak);
    \draw[bend left,dashed, visible on=<12->,alt=<12>{draw=red}{}] (totakeashortbreak) to node [auto] {\scriptsize A} (They);
    \draw[bend left, visible on=<23->] (totakeashortbreak) to node [auto] {\scriptsize D} (short);
\end{tikzpicture}}
\end{frame}
...
\begin{tikzpicture}[every node/.append style={font=\rmfamily}]
    ...
    \node[anchor=west] at (7,0.25) {\small short};
    \node[anchor=west] at (8,0.25) {\small break};
\end{tikzpicture}
\end{minipage}
...
\begin{tikzpicture}[level distance=1cm, sibling distance=1cm, ->,
    ...
    \node(ROOT)[fill=black, circle, visible on=<2->] at (3,0) {}
      child {node (They) {They} edge from parent node [left] {A}}
      child {node {need} edge from parent node [left] {P}}
      child {node (totakeashortbreak) [fill=blue, circle] {}
      {
        child {node {to} edge from parent node [left] {F}}
        child {node (takeabreak) [fill=red, circle] {}
        {
          child {node {take} edge from parent node [right] {C}}
...
\begin{tikzpicture}[->]
    \tiny
    \tikzstyle{main}=[circle, minimum size=7mm, draw=black!80, node distance=12mm]
    \foreach \i/\word in {1/{They},3/{need},5/{to},7/{take},9/{a},11/{short},13/{break}} {
...
\begin{adjustbox}{frame,scale=.75,center}
    \begin{tikzpicture}[level distance=15mm, sibling distance=15mm, ->,
      ...
      \node (ROOT_) at (7,0) [circle] {}
        ...
        ;
      \draw[bend left,dashed,->] (graduation_) to node [auto] {\scriptsize A} (Joe_);
      \draw[bend left,dashed,->,red] (graduation_) to node [auto] {\scriptsize A} (Copenhagen_);
    \end{tikzpicture}
\end{adjustbox}
\vfill
