...
In my research I will be proposing techniques for BSS parsing.
...
I experiment with the English UCCA-annotated corpora \cite{abend2013universal}
...
UCCA's foundational layer, which I use here, covers the predicate-argument
...
I note that the scheme marks one of the incoming edges for each non-root
...
I note that this conversion procedure is simpler than the
...
tree: if a dependency node $t$ is a child of the root in $T_d$, then $t$'s pre-terminal will also be a child of the root node. Otherwise, $t$'s pre-terminal is the child of the pre-terminal associated with $t$'s head in $T_d$. I add an intermediate node in between if $t$ has any dependents in $T_d$,
...
\begin{figure}[ht!]
  ...
  \begin{tikzpicture}[level distance=10mm, ->]
    \node (ROOT) [fill=black, circle] {}
      ...
      ;
  \end{tikzpicture}
  \caption{Example for conversion to a constituency tree.}
\end{figure}
...
\depedge{5}{7}{A}
\end{dependency}
  \caption{Example for conversion to a dependency tree.}
\end{figure}
...
I follow previous work in transition-based constituency parsing \cite{sagae2005classifier}, and include the \textsc{Node} transition for creating new non-terminal nodes.
...
I conduct our main experiments on the UCCA Wikipedia corpus (henceforth, \textit{Wiki}),
...
similar.
I use passages of indices up to 655
...
While UCCA edges can cross sentence boundaries, I adhere to the common
...
I also discard linkage nodes and edges, as they often express inter-sentence
...
In the out-of-domain experiments, I apply the same parser
...
I report two variants of this measure, one where I consider only non-remote edges,
and another where I consider remote edges. I note that the measure collapses to the standard
...
I note that \textsc{uparse} uses beam search by default,
...
\paragraph{\textsc{bsp}.}
I train \textsc{bsp} for 16 iterations, using a \textsc{MinUpdate} value of 5 and a constant learning rate of 1.
...
Currently, I handle these decisions by crude heuristics, but a more sophisticated way can be devised. These will be reflected by higher scores for the oracle experiments.
...
In terms of machine learning, I use a linear classifier with hand-crafted features. As the experiment with the LSTM parser shows, implementing a parser based on neural networks should also give a significant improvement to our parsing scores.
